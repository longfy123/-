...
class CleanDataset():
    def __init__(self, config):
        ...
        self.data_type = config.data.get('data_type', 'original')  # get data type
    ...
    def read_data(self):
        if self.data_type != 'original':
            import os
            base_path = os.path.dirname(self.feature_file)
            filename = f'flow_{self.data_type}.npy'  # flow_residual.npy, flow_trend.npy, flow_seasonal.npy
            feature_file = os.path.join(base_path, filename)
            print(f"Loading {self.data_type} data: {feature_file}")

            if not os.path.exists(feature_file):
                raise FileNotFoundError(f"{self.data_type} data file not found: {feature_file}")
        else:
            feature_file = self.feature_file
            print(f"Loading original data: {feature_file}")
        
        if 'PEMS' in self.data_name:
            data = np.expand_dims(np.load(feature_file)[:, :, 0], -1)
...
class TrafficDataset(Dataset):
    ...
    def get_idx_lst(self):
        ...
        step = self.T_h + self.T_p  # non-overlapping window

        for label_start_idx in range(start, end, step):
